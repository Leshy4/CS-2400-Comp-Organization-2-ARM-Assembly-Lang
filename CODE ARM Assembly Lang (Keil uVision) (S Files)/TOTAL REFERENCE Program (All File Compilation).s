...
;;;;;;;;;;  		CONTAINS CODE FOR ->
;; Arrays -> Access & Store values with Memory Address Incrementation (For int's: originAddress[0] + 4 = originAddress[1]) / 
;; Array Searching / Branch Link / LR -> PC (Link Register & Program Counter) /
;; Check String for Letter 'H' / Flags (Negative,Zero,Carry,Overflow) / Math (Polynomial Eqn) / 
;; Factorial ( Nieve -> Modified -> Link Register (LR) to fill the Program Counter (PC))
;; Suffixes / Swap Contents with EOR->(XOR)/ Floating Point Operations & Area of Circle with Floats
;; char[] = Strings access with by Address Incrementation [Size == Data Type] (Find Value in Array)  
;; Looping in Arrays & more Memory Address manipulation / Memory (Retrieval & Storage == LDM & STM) / <- Load/Store Types: Pre-Indexing (Base Updated||Base Unchanged) / 
;; Post-Indexing Base Updated / Pushing & Popping the Stack (w/Stack Pointer='SP')
;; Name Length / OddSum (Read Array & Sum += Only Odd Numbers & Store in R3) / 
;; Nested Function Calls / Arithmetic Shift Right & Rotate Right / 
;; Copy Bits & Bit Masking & More Stack (Final Code Exam) / Parity Bit (1 Bit for Error Detection);;~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
Reset_Handler
;;~~~~~~~~STANDARD BEGINNING CODE (End)~~~~WRITE CODE BELOW HERE~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~CODE BELOW HERE~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

;;~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~	
;;CW2~~~Find Product of Array elements & Store product in Memory~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
;	LDR r0,=a				;int = 4 Bytes (32 Bit)		
;	LDR r1,[r0,#0]	;1		;int a[3] = {1, 3, 5};		a[0]'s Memory ADDRESS = 1000;
;	LDR r2,[r1,#4]	;3		;int a[]={1000,1004,1008} <-Addresses	
;	LDR r3,[r2,#4]	;5
;	MUL r1,r1,r2
;	MUL r1,r1,r3		
; a DCD 1,3,5			;1000=1	/	1004=3	/	1008=5

;;~~~~~~~~~~~~~~~Read(Search) an Array & check if a # is in the Array.~~~~~~~~~~~~~~~~~
...
;;~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
;;---------BL = Branch Link	== Function call (Returns to same spot (PC) after Call Complete)  -------------------------------------------------------
;	MOV R0, #1	
;	MOV R2, #3	
...
;;~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
;;~~~~~~~~~~~~~~~~FLAGS~~~~~~stored in PSR(Program)~~~~~~~~~~~~~~~~~~~~; 'N'egative/'Z'ero/'C'arry/O'V'erflow
; 	MOV 	 	R2,#0x01 	 	; R2 = 01
; 	MOV 	 	R3,#0x02 	 	; R3 = 02 
; 	ADD 	 	R1,R2,R3 	 	; R1 = 03 
...
;;	Name: Christopher Welch		;;	FunSolve.s		;;	Date: 02/20/20
...
;	LDR R4,=a			=R4=3
;	LDR R1,[R4]			=R1=3	=n
;	MOV R3,#1			=R3=1	=Factorial total variable
;loop1
;	SUB R2, R1, #1		=R2 = n-1
;	CBZ R2, exit		=(R2==0?) exit : continue;
;	MUL R3,R3,R1		= R3 *= n
;	SUB R1,R1,#1		=n-1
;	B loop1				
;exit
...
;;------------------ Factorial Modified
;	MOV r3,#5		;r3 = 5	= n
;	MOV r2,#1		;r2 = 1	= Factorial total variable
;loop
;	CMP r3,#0		;if(n>0?) {				;(n>0?) = Flag that carries
;	MULGT r2,r3,R2	;	fTotal *=n ; 		;    to the next iteration.
;	SUBGT r3,r3,#1	;	n-1;
;	BGT loop		;	loop(); }
...
;	LDR R1,[R4]
;	TEQ R0,R1		;Z=1 
;	St1 DCB "one"	;a DCD 4,2,3,1	== int[] w/ Memory Addresses of:
;	St2 DCB "one"	;(Base Address)1000, 1004, 1008, 1012
;;~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
;	LDR r0,=a
;LOOP
;	LDR R1,[R0,#0] 	;#0 means the OFFSET = a[0]	
;	ADD R2,R2,R1		;R0= 1000 + 4n
;	ADD R0,R0,#4
;	ADD R3,R3,#1
;	CMP R3,#4
;	BLT LOOP
...
;;~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
;;~~~~~~~~~~~~~~~Read an Array & check if a # is in the Array with Memory Addresses
;	LDR r10,=#5		;Looking for #5
;	LDR r11,=#0		;Incrementer
;	LDR r0,=a		;Base Address Loaded
;	LDR r1,[r0,#0]	;1st Number	& Register to READ
...
;	MOV R1,#4
;	LDR	R5,=0X20000018
;	BL	Fun1				;Call Fun1(), calling Fun2(), Return to Fun1() call, Jump to STOP
;	B	stop
;Fun1
;	MOV R2,#2
;	STMFD R5!,(LR)
...
;;~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
;;~~FINAL~~~~~~~~~Copy Bits with Bit Masking & Stored to Memory~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
;	LDR R0, A
;	MOV R3, R0		;;Copy of A		
;	MOV R1, #0x3FF	;;AND for 0-9	 The Bit-Maskers
;	MOV R2, #0xFFF	;;AND for 0-11	 The Bit-Maskers
;	
...
;	MOV R1, #0x4E		;0100 1110	;EVEN PARITY=LSL Bits-> ;TO 0 1001 1100
;	MOV R2, R1	;Copy to LSR		
;	MOV R3, R1
;	MOV R5, #8	;# of bits to read
;	MOV R6, #0	;Total Bits with '1'	
;	B LOOP
;LOOP					;Nested Function calls
;	CMP R5, #0			;(i == 0? finisher() : continue)
;	BEQ finisher		
;	AND R7, R2, #1		;Bit in First position == 1?
;	BL  checkBit	
;	LSR R2, #1
;	SUB R5, R5, #1		;i--
;	B LOOP	
